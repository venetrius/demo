...
public interface ArgumentRepository extends JpaRepository<Argument, Long> {
    ...
    long countByDiscussionIdIn(List<Long> discussionIds);

    @Query("SELECT a.id FROM Argument a JOIN a.discussion d JOIN UsersDiscussion ud ON ud.discussion = d WHERE ud.user.id = :userId AND d.status = arguewise.demo.model.Discussion.DiscussionStatus.ACTIVE AND ud.side = a.author.side")
    List<Long> countByUserAndStatus(User user, Discussion.DiscussionStatus active);
}
